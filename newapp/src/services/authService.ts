...
class AuthService {
  ...
  async register(data: RegisterData) {
    ...
    try {
      ...
      if (response.data && response.data.token) {
        await this.storeAuthData(response.data.token, response.data.user, 'customer');
      }
      return response;
    } catch (error) {
      throw error;
  ...
  // Customer login - First time login requires OTP
  async login(data: LoginData) {
    const response = await api.post('/auth/login', data);
    // response is the unwrapped backend response
    // response structure: { success, message, data: { user, token, requiresOtp } }
    // so response.data = { user, token, requiresOtp }
    // If requiresOtp is true, don't store token yet - wait for OTP verification
    if (response.data && response.data.token && !response.data.requiresOtp) {
      await this.storeAuthData(response.data.token, response.data.user, 'customer');
    }
    return response;
  }

  // Login OTP - Send OTP for first-time login
  async sendLoginOtp(mobile: string) {
    const response = await api.post('/auth/login/send-otp', { mobile });
    return response;
  }

  // Login OTP - Verify OTP and complete login
  async verifyLoginOtp(mobile: string, otp: string) {
    const response = await api.post('/auth/login/verify-otp', { mobile, otp });
    // After OTP verification, store auth data
    if (response.data && response.data.token) {
      await this.storeAuthData(response.data.token, response.data.user, 'customer');
    }
    return response;
  ...
  async storeAuthData(token: string, user: any, role: string) {
    await AsyncStorage.setItem('token', token);
    await AsyncStorage.setItem('user', JSON.stringify(user));
    await AsyncStorage.setItem('userRole', role);
    // Store token expiration (7 days from now)
