...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  ...
  const addToCart = React.useCallback(async (item: Omit<CartItem, 'quantity'>, quantityToAdd: number = 1) => {
    ...
    // If user is logged in, sync with backend in background (non-blocking)
    if (userId) {
      // Fire and forget - don't await to prevent UI blocking
      cartService.addToCart(item.id, quantityToAdd).catch((error) => {
        // Only log errors, don't revert UI changes for better UX
        // The cart will sync on next app open or manual refresh
        if (__DEV__) {
          console.error('Failed to sync cart to backend (non-blocking):', error);
        }
      });
    }
    
    // Return resolved promise immediately for non-blocking behavior
